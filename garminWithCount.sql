...
ifnull( s.total_allocated_stock ,0) as allocated_stock,
(s.total_stock - (d.total_demand - s.total_allocated_stock ) ) as balance
FROM `garmines`
LEFT  JOIN ( 
	SELECT garmines_id, ifnull( SUM(stocks.stock),0 ) as total_stock,
    ifnull( SUM(stocks.allocated_stock ),0 ) as total_allocated_stock    
    FROM stocks
